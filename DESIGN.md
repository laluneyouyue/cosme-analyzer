...
変更したい対象ごとに、開くファイルが1つに決まる状態を目指しています。
...
**「この成分とこの人の相性を0〜100で答えて」と聞いてはいけません。**

73点と78点の違いを定義した人が誰もいないからです。基準が無い以上、LLM が返せるのは「それっぽい数字」だけで、次の3つが同時に起きます。

- 同じ画像を2回投げると違う点数が出る
- 「なぜ73点なのか」を誰も説明できない
- レーダーの5軸も、何点が何を意味するのか定まらない

**これはモデルを上位に変えても解決しません。** より一貫性のある勘になるだけで、根拠は生まれないためです。

### 境界の引き方

```mermaid
flowchart LR
    A1["成分"] --> A2["LLM<br/>0〜3で分類する"] --> A3["Python<br/>計算する"] --> A4["点数<br/>根拠あり・毎回同じ"]
```
...
レーダー上では同じ形で並びますが、意味が違います。「保湿70」は人によって良し悪しが変わりますが、「安全性70」は誰にとっても同じ意味です。混ぜると図が読めなくなるので、独立した**刺激リスク（低／中／高）**として扱います。
...
軸スコア  = min(100, raw / AXIS_FULL_SCORE[軸] × 100)
```

満点の基準は軸ごとに違います。基準の 6.0 は「最も強い成分(3)が上位に2つあれば満点」という想定です。
...
**保湿だけ重くしているのは、軸によって成分の「供給量」がまるで違うためです。** 保湿成分（グリセリン・BG・各種オイル）はほぼ全ての化粧品に入っているので、全軸を同じ基準で測ると保湿だけ毎回100点に張り付きます。

| 軸 | 重み付き合計 `raw` の実測（6製品） |
|---|---|
| 保湿 | **15.7** / 7.2 / 6.2 / 4.1 / 1.0 / 1.0 |
| 鎮静 | 2.0 / 3.0 / 5.0 / 0 / 3.0 / 0 |
| ハリ | 1.7 / 3.0 / 0 / **5.4** / 0 / 0 |
| 透明感 | 1.0 / **7.0** / 1.0 / 0.4 / 0 / 0 |
| 毛穴 | 0 / 2.0 / 0 / 0 / **5.0** / 0 |

保湿以外は上限が 5.0〜7.0 に収まるのに、保湿だけ 15.7 まで伸びます。**「保湿70点」と「透明感70点」を同じ意味の70点にするための正規化**です。透明感70点は狙って配合しないと出ませんが、保湿70点は珍しくありません。

#### ② 採点に使う軸を決める

```python
weight[軸] = 1.0 if 重視する効果に含まれる else 0.0
# ひとつも選んでいない場合だけ、5軸すべて 1.0（＝総合力として見る）
```

**重み0の軸は、高くても低くても点数に一切影響しません。**

**重み0の軸を計算から外すのが、この式の要点です。** 5軸すべてに「求める水準」を置いて重なりを測ると、求めてもいない軸を満たせないことが減点として効きます。保湿だけを求めている人に保湿100点のクリームを見せても、他の4軸が低いせいで100点にならない、という状態になります。**「求めていないものができていない」のは、その人にとって欠点ではありません。**

同じ理由で、年齢も**採点に使っていません**。年代で軸を勝手に足すと、本人が選んでいない軸で減点することになるためです。かわりに情報提示だけに使います。

| 年齢層 | 提案する軸（点数には影響しない） |
|---|---|
| 10代 / 20代 | 毛穴・皮脂ケア |
| 30代 | ハリ・エイジングケア |
| 40代 / 50代以上 | ハリ・エイジングケア、保湿・うるおい |

結果画面には「40代の方は『ハリ・エイジングケア』もよく重視されています（この製品は92点）。点数には含めていません」と出すだけです。**選ぶかどうかは本人に任せます。**

#### ③ 相性スコア

```python
base = Σ(製品[軸] × weight[軸]) / Σ weight[軸]
...
重みが0か1しかないので、**この式は「選んだ軸の平均点」そのもの**です。

レーダーチャートで★の付いた軸の値を平均すれば、その数字になります。図と点数の対応は1本のレーダーでも保たれています。
...
表示順は**配合量の多い順**（＝成分表に書かれている順）です。注意すべき成分を上に並べ替える案もありますが、**手元の容器と見比べたときに並びが違うと照合できません。** 「成分表は配合量の多い順に並ぶ」という、このアプリ自体の前提とも食い違います。

注意すべき成分への導線は、並び順以外に用意しています。

- 刺激リスクのカードに、根拠になった成分名を出す
- 避けたい成分に当たった件数を、点数の理由文に書く
- リスト内でも色分け（赤／緑／灰）で区別する

**上位何件かに絞るのではなく、全件を出してスクロールに畳む**方針です。20件で切ると21番目の重要な警告成分が消えるため、情報は捨てません。
...
| ③ | **でっちあげ** | 成分表にない成分を追加 | ✅ 実装済み | 容易 |
| ④ | **取りこぼし** | 20成分あるのに8個しか返さない | ✅ 実装済み | 容易 |
...
対策は `verification.py` にまとめてあります。

```
Step 1 (Vision)  画像 ──► ingredients_text        ← ここで真実が確定する（＝原典）
                                │
Step 2 (分類)                   ▼
                 ingredients_text ──► [{name, original_name, ...}, ...]
                                      │
                 verification.filter_hallucinated() ◄─┘
                                      │
                                      └─ 原典に無い成分を落としてから採点へ
```
...
**採点より前に実行します。** 存在しない成分が点数に混ざったあとでは、どの点数がその成分由来かを取り消せないためです。

#### 照合をゆるくしている理由

**完全一致では判定できません。** 実際の成分表は表記の揺れが大きく、厳密に照合すると**本物を消してしまいます**。

| 原典の表記 | LLM の出力 | 完全一致 | 実装した判定 |
|---|---|---|---|
| `水酸化ナトリウム` | `水酸化Na` | ❌ 不一致 | ✅ 一致 |
| `ヒアルロン酸Ｎａ` | `ヒアルロン酸Na` | ❌ 不一致 | ✅ 一致 |
| `トコフェロール` | `レチノール` | ❌ 不一致 | ❌ 不一致（＝正しく除外） |

やっていることは3つだけです。

1. **正規化** — NFKC で全角半角をそろえ、小文字化し、記号と空白を落とす
2. **部分一致** — そのまま含まれていれば一致
3. **2文字並びの一致率** — 含まれていなければ、`ヒアルロン酸 → {ヒア, アル, ルロ, ロン, ン酸}` のように2文字ずつに切って、原典とどれだけ共通するかを見る。6割以上（`VERIFY_MATCH_THRESHOLD`）で一致とみなす

3文字以下の短い成分名は偶然一致しやすいので、部分一致でしか認めません（`VERIFY_EXACT_ONLY_LENGTH`）。

#### 安全弁：照合が破綻したら何もしない

LLM が返した成分のうち**半分も原典と一致しなかった場合**（`VERIFY_MIN_MATCH_RATIO`）、疑うべきは LLM ではなく照合のほうです。原典の読み取り結果が想定外の形（商品名だけ、説明文だけ、など）だった可能性が高い。

このとき照合結果は破棄し、**1件も除外しません**。

> **でっちあげを1件見逃すより、本物を1件消すほうが害が大きい。**
> 見逃しは点数が少しずれるだけですが、本物を消すと「成分表に載っているのに画面に出てこない」という、利用者から見て明確な嘘になります。

### 8.4 ④ 取りこぼしへの対策（実装済み）

③と同じ照合を**逆向き**に回すだけで、取りこぼしも測れます。

| | 見る向き | 問い |
|---|---|---|
| ③ でっちあげ | 出力 → 原典 | 無いものを足していないか |
| ④ 取りこぼし | 原典 → 出力 | あるものを落としていないか |

原典を区切り文字（`、` `,` `・` `/` 改行）で分割し、1件ずつ出力側に見つかるかを調べます。見出し語（`全成分` など）と、意図的に除外している水は数えません。

```python
取りこぼし率 = (原典の成分数 - 出力で見つかった数) / 原典の成分数
```

30%（`OMISSION_WARN_RATE`）を超えたら「読み取りが不安定」と判定し、撮り直しを促す文言を結果に含めます。**幻覚より取りこぼしのほうが頻度が高い可能性があります。**
...
③④で得られた数字は、内部で捨てずに `reliability` として結果に含めています。

```json
"reliability": {
  "source": "image",       // image=成分表を読んだ / web=検索した
  "verified": true,        // 原典と照合したか
  "source_count": 18,      // 原典から数えた成分数
  "listed_count": 16,      // 解析結果に載った成分数
  "dropped_count": 1,      // 原典に無く除外した件数
  "omission_rate": 11,     // 取りこぼし率(%)
  "warnings": ["..."]      // 画面に出す注意文
}
```

`warnings` の文言は Python が組み立てます。LLM に書かせると、**警告そのものが幻覚になりうる**ためです。

画面側は `ReliabilityNote.tsx` がこれを並べるだけで、独自の判断はしません。注意文があるときだけ色を付け、無いときは内訳を淡く出します（「問題なし」を強調しても情報量がないため）。

> 精度を上げる努力と、**うまくいかなかったときに正直に言う仕組み**は別物です。
> 前者には上限がありますが、後者は実装すれば必ず効きます。
...
**理由**：Cloudflare Pages の環境変数は Production と Preview で別枠です。「未設定なら通す」実装にすると、Preview 環境（ブランチを push すると自動生成される）が**認証なしで公開されます**。
...
LLM は成分表から「ブルベ夏に合うか」を判定する手段を持ちません。プロンプトに渡すと、**もっともらしいだけの文章を生成させる原因**になります。
...
| **出力を外部の真実と照合する** | 原典照合（`verification.py`） |
