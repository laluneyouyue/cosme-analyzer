...
当初は「`original_name` が原典に含まれるか」という完全一致を考えていましたが、実際の成分表は表記の揺れが大きく、そのままでは**本物を消してしまいます**。
...
| 2 | 原典照合ガード（③対策） | ✅ 実装済み（`verification.py`） |
| 3 | 取りこぼし検出（④対策） | ✅ 実装済み（`verification.py`） |
| 4 | 信頼度の表示（`source` と欠落率） | ✅ 実装済み（`ReliabilityNote.tsx`） |
| 5 | フロントエンドの対応（2重レーダー・製品名・刺激リスク・年齢層） | ✅ 実装済み |
